...
// 动态导入 VIP 图标
const VipIcon = dynamic(() => import('@/components/icons/VipIcon'), {
  ssr: false
})
...
function DesktopNavItem({ href, children }: { href: string; children: React.ReactNode }) {
  ...
  useEffect(() => {
    setMounted(true)
    setIsVip(!!localStorage.getItem('vip_code'))
  }, [])

  const isLockedVip = href === '/vip-blog' && !isVip
      ...
      <ul className="flex rounded-full px-3 text-sm font-medium bg-card ring-1 ring-muted shadow-md backdrop-blur">
        {navItems.map((item, index) => (
          <Fragment key={item.name}>
            ...
            <DesktopNavItem href={item.href}>{item.name}</DesktopNavItem>
          </Fragment>
        ))}
      </ul>
    </nav>
